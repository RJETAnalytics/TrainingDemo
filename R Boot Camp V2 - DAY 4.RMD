...
NumberOnes_DF %>%
  ggplot(aes(danceability, WeeksAt1)) +
    geom_point() +
    geom_line(aes(danceability, NumberOnes_LM$fitted.values), color = "red", size = 4) +
...
plot(NumberOnes_DF %>%
       select(WeeksAt1, valence, danceability, spotify_track_popularity))
...
Summary_PopTest = Popularity_DF_Test %>%
  ...
  summarize(MAE = mean(abs(Err)),
            .groups = "drop") %>%
  mutate(MAPE = MAE / mean(Popularity_DF_Test$spotify_track_popularity))
...
TestDF = tibble(Cheater = c("Larry", "Moe", "Shemp", "Curly"))
